...
//			logger.info("Pattern has been matched "+ sparkwaveNetwork.getReteNetwork().getNumMatches()+ " times.");
	...
	private Triple getTriple(){
		...
		try{
			...
			logger.error(e);
			run = false;
		} catch (SparkwaveEndOfStreamException e) {
			run = false;
		}
	...
	private int getChar() throws IOException, InterruptedException, SparkwaveEndOfStreamException{
		int c;
		if((c = streamReader.read()) != -1)
			return c;
		else
			throw new SparkwaveEndOfStreamException();
	}
