...
public class RETENetwork {
	...
	private AlphaMemory buildOrShareAlphaMemory(TripleCondition condition){
		...
		//TODO Not all alpha nodes should have memory, only those which are connectives to beta memory
		if (currentNode.getOutputMemory() == null){
	...
	private AlphaNode buildOrShareConstantTestNode(AlphaNode currentNode, RDFTriple.Field field, String lexicalTestSymbol){
		...
		for (AlphaNode childNode : currentNode.getChildren()){
			...
			if (((childNode instanceof ConstantSubjectTestAlphaNode) && (field.equals(RDFTriple.Field.SUBJECT))) ||
				((childNode instanceof ConstantPredicateTestAlphaNode) && (field.equals(RDFTriple.Field.PREDICATE))) ||
				((childNode instanceof ConstantObjectTestAlphaNode) && (field.equals(RDFTriple.Field.OBJECT))))
				if (((ValueTestAlphaNode)childNode).getTestValue().equals(lexicalTestSymbol))
					return childNode;
		}
...
//	}
	...
	public void printAlphaNode(AlphaNode alphaNode, String prefix){
		...
		logger.debug(prefix + "Field to test:" + (alphaNode.getClass().getName()));
		if (alphaNode instanceof ValueTestAlphaNode)
			logger.debug(prefix + "Value to test:" + ((ValueTestAlphaNode)alphaNode).getTestValue());
